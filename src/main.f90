...
! Strong modifications of the main by S. Gavino in 2024
!
! INPUT FILES
! 
...
subroutine interpet_command_line(gotit,fromstdi,quit)
  ...
  do while(iarg.le.numarg) 
    ...
    if(buffer(1:3).eq.'run') then
      !
      ! Run the main chemical scheme
      ! This computes the chemical evolution in the model
      !
      do_chemical_scheme = .true.
      gotit = .true.
      elseif(buffer(1:7).eq.'outputs') then
        !
        ! Generate outputs in ASCII
        !
        do_outputs = .true.
        gotit = .true.
      elseif(buffer(1:5).eq.'rates') then
        !
        ! Run the main chemical scheme
        ! This computes the chemical evolution in the model
        !
        do_rates = .true.
        gotit = .true.
        elseif(buffer(1:5).eq.'major') then
          !
          ! Run the main chemical scheme
          ! This computes the chemical evolution in the model
          !
  ...
  if(.not.gotit) then
    ...
    write(stdo,*) '  major      : Find the major reactions for each time and species'
    quit = .true.
  endif
