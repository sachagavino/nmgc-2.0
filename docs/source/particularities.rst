...
To avoid these issues, in astroMUGS, we made sure the user can define a floor value of Av and gas number density. 
...
We typically recommand a safe choice of ``Av >= 1 MAG`` and ``nH >= 10^2 cm⁻³`` to be on the safe side.
Basically, at 10^3–10^4 cm⁻³ the chemistry is still diffuse enough compared to the disk but tractable and will prevent convergence issues.
